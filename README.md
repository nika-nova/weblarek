...
- 2 интерфейса для представления данных о товаре и покупателе
...
Поля класса:  
`payment: TPayment | null = null` - хранит способ оплаты  
...
`validate(): BuyerValidationErrors` - валидация данных. Поле является валидным, если оно не пустое.
...
События:
`preview:click` - передаётся в презентере через onClick
...
События:
`cart:itemDeleteClick` - передаётся в презентере через onDelete
...
### Презентер
---
Представлен в рамках основого файла main.ts.
...
`preview:click` нажатие кнопки покупки / удаления товара из корзины (в карточке товара)  
